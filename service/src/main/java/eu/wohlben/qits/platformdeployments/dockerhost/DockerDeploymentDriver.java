...
import eu.wohlben.qits.platformdeployments.deployments.control.DeploymentIdentifiers;
import eu.wohlben.qits.platformdeployments.deployments.control.PdProcess;
...
public class DockerDeploymentDriver implements DeploymentDriver {
  ...
  @ConfigProperty(name = "qits.pd.container-runtime")
  String runtime;

  @ConfigProperty(name = "qits.pd.pull-timeout-seconds")
  long pullTimeoutSeconds;

  @ConfigProperty(name = "qits.pd.health-interval-seconds")
  long healthIntervalSeconds;

  @ConfigProperty(name = "qits.pd.health-retries")
  int healthRetries;

  @ConfigProperty(name = "qits.pd.health-start-period-seconds")
  long healthStartPeriodSeconds;

  @ConfigProperty(name = "qits.pd.output-max-chars")
  int outputMaxChars;

  /**
   * The prefix of the per-application run-argument family: {@code
   * qits.pd.run-args.<application-name>} holds extra {@code docker run} arguments (volumes, env,
   ...
   */
  static final String RUN_ARGS_PREFIX = "qits.pd.run-args.";
  ...
  @ConfigProperty(name = "qits.pd.docker-socket-path")
  ...
  List<String> buildArgv(StartSpec spec) {
    ...
    // The deployment's own additions for this application — qits.pd.run-args.<name>, whitespace
