...
public class EnvironmentOperations {
   ...
   * label-driven — it reaps by {@code qits.pd.environment} and removes the networks carrying that
   * id — so it needs nothing from the topology, but deleting the tier first would leave a failed
   * teardown with no row to retry it from. Deleting last means a half-finished teardown is still
   * addressable.
   *
   ...
   * {@code qits.pd.legacy-network} as its bundle — the dev tier IS that case, its bundle is {@code
   * qits-net} — and it is not that environment's to take away: it is the transition membership of
   * every container on the host, platform services included. Disconnecting them from it would cut
   * qits-idp off from the platform, and this component would be doing it to itself mid-request. So
   * it is skipped for both steps, and the environment's derived per-application networks still go.
   */
