 ...
 *
 * <p><b>The labels are {@code qits.pd.*}, and what carries {@code qits.cd.*} is not this
 * component's.</b> Containers and networks a retired qits-cd left behind are treated exactly like
 * unlabelled ones — adoptable predecessors, never protected — because that is what makes the
 * cutover to this component a deployment rather than a flag day. Nothing here reads a
 * {@code qits.cd.*} label, and nothing should start to: the absence of a {@code qits.pd.*} label is
...
public interface DeploymentDriver {
  ...
  String ENVIRONMENT_LABEL = "qits.pd.environment";

  String APPLICATION_LABEL = "qits.pd.application";
  String DEPLOYMENT_LABEL = "qits.pd.deployment";

  /** {@code environment} or {@code platform} — what a reconciliation looks a container up by. */
  String TARGET_LABEL = "qits.pd.target";

  /** {@code true} on an environment's public nodes — the other half of the reconciliation lookup. */
  String AVAILABLE_ON_ENV_LABEL = "qits.pd.available-on-env";

  /** On networks: {@code bundle}, {@code application} or {@code platform}. */
  String NETWORK_LABEL = "qits.pd.network";

  /** On containers and on per-application networks: whose it is. */
  String APP_NAME_LABEL = "qits.pd.app-name";
